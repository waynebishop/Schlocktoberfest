...
abstract class DatabaseModel
{
	...
	public function __construct($input = null){
		...
		if(is_integer($input) && $input > 0){
			//if input is number, Load that record from DB
			$this->find($input);
		}
		if(is_array($input)){
	...
	public static function all($sortcolumn = "", $asc = true, $pageNumber=null, $pageSize=null){
		...
		if($sortcolumn){
			if(! array_search($sortcolumn, static::$columns)){
				throw new UnexpectedValueException("Property $sortcolumn not found in the columns list.");
			}
		...
		//Getting Comments on the Single movie view
			...
			foreach ($rules as $rule) {
				...
				switch ($rule) {
					...
					case 'maxlength':
						if(strlen($this->$column) > $value){
							$valid = false;
							$this->errors[$column] = "Must no more than $value characters long.";
						}
						break;
					case 'numeric':
					...
					case 'exists':
						try{
							...
							$valid = false;
							$this->errors[$column] = "This value doesnt exists";
							break;
						}
